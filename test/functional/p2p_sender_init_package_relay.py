...
class PackageRelayTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        self.log.info("Test that we disconnect the peer if pkgtxns without negotiating sendpackages")
        self.test_disconnect_if_no_sendpackages()
    ...
    @cleanup
    def test_disconnect_if_no_sendpackages(self):
        self.restart_node(0, extra_args=["-packagerelay=0"])
        package_receiver = self.nodes[0]
        package_sender = self.nodes[0].add_p2p_connection(PackageRelayer(send_sendpackages=False))
